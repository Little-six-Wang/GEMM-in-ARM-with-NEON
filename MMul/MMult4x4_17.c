...
void AddDot4x4(int, double*, double*, double*, int);
...
void InnerKernel(int m, int n, int k, double* a, int lda, double* b, int ldb, double* c, int ldc, int flag){
    ...
    for(j = 0; j < n; j += 4){
        ...
        for(i = 0; i < m; i += 4){
            ...
            AddDot4x4(k, &packedA[i * k], &packedB[j * k], &C(i, j), ldc);
        }
    }
}
...
void AddDot4x4(int k, double* a, double* b, double* c, int ldc){
    int p;
    ...
    for(p = 0; p < k; p++){
        a_0p_1p_vreg = vld1q_f64(a);
        a_2p_3p_vreg = vld1q_f64(a + 2);
        a += 4;

        b_p0_vreg = vld1q_dup_f64(b);
        b_p1_vreg = vld1q_dup_f64(b + 1);
        b_p2_vreg = vld1q_dup_f64(b + 2);
        b_p3_vreg = vld1q_dup_f64(b + 3);
        b+= 4;
